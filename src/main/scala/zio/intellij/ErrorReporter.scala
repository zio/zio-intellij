...
import zio.intellij.utils.{ProjectSyntax, StringBuilderSyntax, Version}
...
object ErrorReporter {
  def reportErrorOnGithub(
    ...
    errors: List[String],
    versions: List[(Version, ScalaVersion)],
    label: String
  ) = {
    def enc(s: String) =
    ...
    appendAdditionalInformation(sb, versions)
  ...
  private def appendAdditionalInformation(sb: StringBuilder, versions: List[(Version, ScalaVersion)]) = {
