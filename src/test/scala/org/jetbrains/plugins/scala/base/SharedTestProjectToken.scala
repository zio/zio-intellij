...
/**
 *  Test cases with the same Some(token) will share test project if run one by-one.<br> This can make each test case
 *  initialization significantly faster.<br> If you do not want the project to be shared the token should be None or
 *  some other unique value.
 *
 *  @note
 *    Suppose test classes A and B use token T1, and test C uses token T2.<br> If test are run in following order: A, C,
 *    B, then project will not be reused between A and B. (This is because under the hood IntelliJ platform uses a
 *    singleton for storing current test project)
 */
